...
{
    return [self.imageCache diskImageExistsWithKey:url];
}

-(id<SDWebImageOperation>)downloadWithURL:(NSURL *)url options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock storeImage:(ResizeImageObject *(^)(UIImage *))onStoreImage completed:(SDWebImageCompletedWithFinishedBlock)completedBlock
{
            ...
            id <SDWebImageOperation> subOperation = [self.imageDownloader downloadImageWithURL:url options:downloaderOptions progress:progressBlock completed:^(UIImage *downloadedImage, NSData *data, NSError *error, BOOL finished) {
                if (weakOperation.isCancelled) {
                    ...
                    // NOTE: We don't call transformDownloadedImage delegate method on animated images as most transformation code would mangle it
                        ...
                        ResizeImageObject *imgStore=nil;
                        if(onStoreImage)
                            imgStore=onStoreImage(downloadedImage);
                        
                        dispatch_main_sync_safe(^{
                            completedBlock(imgStore.resizeImage?:downloadedImage, nil, SDImageCacheTypeNone, finished);
                        });
                        
                        if (downloadedImage && finished) {
                            [self.imageCache storeImage:downloadedImage recalculateFromImage:NO imageData:data forKey:key toDisk:cacheOnDisk];
                            if(imgStore && imgStore.resizeImage)
                                [self.imageCache storeImage:imgStore.resizeImage forKey:imgStore.key toDisk:true];
                        }
                    }
                }
...
@implementation ResizeImageObject
