       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    [self decodeImage:scrn];
//    [self performSelectorInBackground:@selector(decodeImage:) withObject:scrn];
}

-(void) decodeImage:(UIImage*) img
{
    Decoder* d = [[Decoder alloc] init];
    d.readers = readers;
    d.delegate = self;
    
    decoding = [d decodeImage:img] == YES ? NO : YES;
    ...
    if (decoding) {
        ...
        UIImage *scrn = [[[UIImage alloc] initWithCGImage:img.CGImage
                                           scale:1.0
